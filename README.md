...
面向生产的货运账单微信小程序前端。当前版本使用本地 Mock 数据，后端接口暂未接入。

## 已实现

- 账单列表、状态切换、全局搜索与同页高级筛选
- 当前筛选列表内批量管理、永久删除与二次确认
...
Mock 数据保存在 `services/mock-store.js`，页面统一通过 `services/bill-service.js` 访问。后续接入后端时替换服务层实现即可，页面不直接依赖存储结构。
