...
容器以 UID/GID `10001` 运行。当前内置的 OCR 原图存储是文件系统。单机部署须将同一持久卷挂载到 API、Worker 和 Maintenance，并确保该用户可读写；本地 Compose 的命名卷不等于跨节点共享存储。多节点部署须先实现并验证共享对象存储适配器，不能使用节点本地目录或仅设置 `OCR_STORAGE_SHARED=true` 代替共享能力。
...
- OCR 原图存储与部署拓扑匹配，API、Worker 和 Maintenance 能访问同一文件；加密、备份及恢复已在目标环境验证
- 若启用 Qwen OCR，已完成数据合规评审、费用告警配置，并在非生产环境验证识别与失败重试
...
- 使用当前文件系统实现时，OCR 原图所在持久卷须加密并纳入备份与恢复演练；单机各角色共享同一卷。多节点使用共享对象存储适配器时，须验证版本控制和服务端加密。应用维护任务按配置的保留期清理原图，默认 30 天。
- 至少每季度在隔离环境执行一次恢复演练。
